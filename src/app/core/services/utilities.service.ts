...
export class UtilitiesService {
  ...
  confirmDialog(message: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.confirmationService.confirm({
        ...
        reject: () => {
          resolve(false); // Resolve with false when 'No' button is clicked
        },
        acceptLabel: 'Yes', // Set the label for the accept button to 'Yes'
        rejectLabel: 'No'   // Set the label for the reject button to 'No'
      });
    });
  }
  ...
  downloadTemplate(fileName: string): void {
    const filePath = `assets/${fileName}.xlsx`;
    fetch(filePath)
